...
# (6) model version: which Denmark model is fitted (full version or the simplified version, load 'DenMark-M1.stan' or 'DenMark-M2.stan') 


# Expected output from the function: 
# (1) the sampled from the posterior distribution 
# (2) the Rhat statistics for convergence check 
# (3) the WAIC criteria
...
# dataset: a dataframe with columns 'x', 'y' and 'gene'
# L1: half range for the x axis 
# L2: half range for the y axis
  ...
  grid_area   <- grid_info[[2]] # grid area
  log_grid_area <- log(grid_area)
  N           <- grid_info[[3]] # grid size
  ...
  # distance and the distance-based prior scale
  ...
  input <- list(n = N,
                ...
                y2 = M, # outcome2: marks 
                d = 2, # dim=2
                mstar = mstar, # total number of basis function 
                coords = coords, # the coordinates 
                log_grid_area = log_grid_area, # log(area)
                mrange = mrange, # median range to be assigned for the range 
                indices = S, 
                L = L,
                is_centerted_PHI = 0)
  ...
  # load the DenMark model or the simplified DenMark version
  stan_file <- denmark_version
  # stan_file <- denmark_simple_stan
